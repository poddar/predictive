...
def main(argv):
    ...
    time_stamp=time()
    d={'y_test':np.array(y_test).ravel(),'Predictions':np.array(predictions).ravel()}
    df=pd.DataFrame(d) 
    ...
    Append_2_log(time(),total_time,len(y_train),len(y_test),threshold,auc,0,rig,0,selected_list," ")
