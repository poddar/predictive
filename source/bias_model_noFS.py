...
# ======= BEST FEATURES 
# ==== Train=8M and Test=2M  acoording to AUC Score ========
# ======= OCT 8, 2013 =====
# auc:                 0.806940596473
# rig:                 0.0952802069551
# ==========================================
# average prediction:  0.0023606617466
# min prediction:      0.0
# max prediction:      0.0286020868737
# ==========================================
# average prediction for clicks:  0.00673741946606
# min prediction for clicks:      0.0
# max prediction for clicks:      0.0286020868737
# ==========================================
# average prediction for non-clicks:  0.00234997612835
# min prediction for non-clicks:      0.0
# max prediction for non-clicks:      0.0286020868737
# ==========================================
# GTA train:           0.0032735
# GTA test:            0.0024355
# ===========================================================================
# Best selected_list so far:  ['site', 'account']
# auc:                        0.81289948648
# rig:                        0.105684632372
# ==========================================
# best model:           ['site', 'account']
# threshold:            0
# AUC:                  0.81289948648
# RIG:                  0.105684632372
# AUC log:              [0.81129392889513174, 0.81289948647965637]
# RIG log:              [0.10299390297103272, 0.10568463237156789]
# Mean prediction log:  [0.0030691314779402006, 0.0023606617465987312]
# Max prediction log:   [0.0083530413894829791, 0.028602086873682514]
# done with EVERYTHING in 539.095299min
...
def main(argv):
    ...
    if X_train.getnnz() > 0:
        ...
        predictions_raw,predictions_raw_train,score_test = SGDRegressor_pred(X_train, X_test, y_train_normalized, y_train_mean,y_test)   

        predictions       = [0 if pred < 0 else pred for pred in predictions_raw]
        
        predictions_train = [0 if pred < 0 else pred for pred in predictions_raw_train]  
    ...
    auc_ir                                                      = auc_score(y_test.astype(int).ravel(), predictions_ir) # put y_test.astype(int).ravel() in a variable outside the loop
...
def IsotonicRegression_pred(y_train,predictions_train,test_preds,bin_step,y_test): 
    ...
    y_ir        =   ir.fit_transform(predictions_sorted,
                                     np.array(y_train,float)[predictions_sorted_indexes].ravel())
