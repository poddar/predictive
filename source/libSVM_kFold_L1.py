...
train_num_arr=[8]
cut_off_arr=[0]#,30,100,300,1000,3000,9000,25000,75000]
    ...
    for cut_off in cut_off_arr:
        ...
        for k in range(folds):
            ...
            for c in c_arr:
                ...
                ##########
                ##########
                if selec_criteria == 'auc':
                    if auc_w>best_val_auc:
                        best_val_c=c
                        best_val_auc=auc_w
                        best_val_rig=rig_w
                if selec_criteria == 'rig':
                    if rig_w>best_val_rig:
                        best_val_c=c
                        best_val_auc=auc_w
                        best_val_rig=rig_w
            ...
            train_num=str(tr)
            test_num=str(1)
            AppendLogCSV(analysis_path+'gouped_s-a-sa-KFold_auc_val_sel_'+selec_criteria+'_log.csv','auc',auc_val_arr,train_num,test_num,cut_off,best_val_c,c_arr)
            AppendLogCSV(analysis_path+'gouped_s-a-sa-KFold_rig_val_sel_'+selec_criteria+'_log.csv','rig',rig_val_arr,train_num,test_num,cut_off,best_val_c,c_arr)
        ...
        train_num=str(tr)                        
        test_num=str(1)
        AppendLogCSV(analysis_path+'gouped_s-a-sa-KFold_auc_test_sel_'+selec_criteria+'_log.csv','auc',auc_test_arr,train_num,test_num,cut_off,best_val_c,['f1','f2','f3','f4','f5'])
        AppendLogCSV(analysis_path+'gouped_s-a-sa-KFold_rig_test_sel_'+selec_criteria+'_log.csv','rig',rig_test_arr,train_num,test_num,cut_off,best_val_c,['f1','f2','f3','f4','f5'])
print '###########################'
